      ...
      <div className="account-card__body">
        ...
        <button className="account-sparkline" onClick={onHistory} aria-label="查看历史流量">
          <svg ref={sparklineRef} viewBox="0 0 320 58" preserveAspectRatio="none" aria-hidden="true" onMouseMove={handleSparklineMove} onMouseLeave={() => setHoveredPoint(null)}>
            ...
            {points.length > 0 && <><polygon points={`${points[0].split(',')[0]},48 ${points.join(' ')} ${points.at(-1)!.split(',')[0]},48`} fill={`url(#${gradientId})`} /><polyline points={points.join(' ')} fill="none" stroke="var(--brand)" strokeWidth="2" vectorEffect="non-scaling-stroke" /><circle cx={points.at(-1)!.split(',')[0]} cy={points.at(-1)!.split(',')[1]} r="2.5" fill="var(--brand)" />{hoveredPoint && <><circle cx={hoveredPoint.x} cy={hoveredPoint.y} r="3" fill="var(--brand)" /><rect x={Math.max(6, Math.min(314 - 58, hoveredPoint.x - 29))} y={hoveredPoint.y - 30} width="58" height="22" rx="5" fill="var(--raise)" stroke="var(--line-strong)" strokeWidth="0.5" opacity="0.95" /><text x={Math.max(35, Math.min(285, hoveredPoint.x))} y={hoveredPoint.y - 20} textAnchor="middle" fill="var(--ink)" fontSize="9.5" fontWeight="600">{hoveredPoint.traffic.toFixed(2)} GB</text><text x={Math.max(35, Math.min(285, hoveredPoint.x))} y={hoveredPoint.y - 12} textAnchor="middle" fill="var(--muted)" fontSize="8.5">{hoveredPoint.time}</text></>}</>}
          </svg>
