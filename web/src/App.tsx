      ...
      <div className="account-card__body">
        ...
        <button className="account-sparkline" onClick={onHistory} aria-label="查看历史流量">
          <svg ref={sparklineRef} viewBox="0 0 320 58" preserveAspectRatio="none" aria-hidden="true" onMouseMove={handleSparklineMove} onMouseLeave={() => setHoveredPoint(null)}>
            ...
            {points.length > 0 && <><polygon points={`${points[0].split(',')[0]},48 ${points.join(' ')} ${points.at(-1)!.split(',')[0]},48`} fill={`url(#${gradientId})`} /><polyline points={points.join(' ')} fill="none" stroke="var(--brand)" strokeWidth="2" vectorEffect="non-scaling-stroke" /><circle cx={points.at(-1)!.split(',')[0]} cy={points.at(-1)!.split(',')[1]} r="2.5" fill="var(--brand)" />{hoveredPoint && <><circle cx={hoveredPoint.x} cy={hoveredPoint.y} r="3.5" fill="var(--brand)" stroke="var(--surface)" strokeWidth="2" /><rect x={Math.max(4, Math.min(316 - 70, hoveredPoint.x - 35))} y={Math.max(4, hoveredPoint.y - 32)} width="70" height="24" rx="6" fill="var(--raise)" stroke="var(--line-strong)" strokeWidth="1" /><text x={Math.max(39, Math.min(281, hoveredPoint.x))} y={Math.max(18, hoveredPoint.y - 18)} textAnchor="middle" fill="var(--ink)" fontSize="11" fontWeight="600">{hoveredPoint.traffic.toFixed(2)} GB</text><text x={Math.max(39, Math.min(281, hoveredPoint.x))} y={Math.max(30, hoveredPoint.y - 6)} textAnchor="middle" fill="var(--muted)" fontSize="10">{hoveredPoint.time}</text></>}</>}
          </svg>
